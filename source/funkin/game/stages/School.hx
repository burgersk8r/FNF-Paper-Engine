...
class School extends BaseStage
{
	...
	override function create()
	{
		...
		var bgSky:BGSprite = new BGSprite('stages/week6/weeb/weebSky', 0, 0, 0.1, 0.1);
		add(bgSky);
		bgSky.antialiasing = false;

		var repositionShit = -200;

		var bgSchool:BGSprite = new BGSprite('stages/week6/weeb/weebSchool', repositionShit, 0, 0.6, 0.90);
		add(bgSchool);
		bgSchool.antialiasing = false;

		var bgStreet:BGSprite = new BGSprite('stages/week6/weeb/weebStreet', repositionShit, 0, 0.95, 0.95);
		add(bgStreet);
		bgStreet.antialiasing = false;

		var widShit = Std.int(bgSky.width * PlayState.daPixelZoom);
		if(!ClientPrefs.data.lowQuality) {
			var fgTrees:BGSprite = new BGSprite('stages/week6/weeb/weebTreesBack', repositionShit + 170, 130, 0.9, 0.9);
		...
		if(!ClientPrefs.data.lowQuality) {
			var treeLeaves:BGSprite = new BGSprite('stages/week6/weeb/petals', repositionShit, -40, 0.85, 0.85, ['PETALS ALL'], true);
	...
	function initDoof()
	{
		var file:String = Paths.dialogueTxt(songName + '/dialogue/' + songName + 'Dialogue'); //Checks for vanilla/Senpai dialogue
		#if MODS_ALLOWED
